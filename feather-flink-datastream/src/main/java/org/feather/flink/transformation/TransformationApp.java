package org.feather.flink.transformation;
...
import org.feather.flink.source.AccessSource;
 ...
 * @author: feather
 * @description:将map 算子对应的函数作用到DataStream，产生一个新的DataStream
 * map 会作用到已有的DataStream这个数据集中的每一个元素上
 * @since: 2023-07-29 15:36
 * @version: 1.0
 */
...
//        stream1.union(stream2).print();
    ...
    public static void keyBy(StreamExecutionEnvironment env) {
     ...
     //  mapStream.keyBy("domian").sum("traffic").print();
/*        mapStream.keyBy(new KeySelector<Access, Object>() {
            @Override
            public Object getKey(Access access) throws Exception {
                return access.getDomain();
            }
        }).sum("traffic").print();*/
        mapStream.keyBy(Access::getDomain).sum("traffic");
    }
    /**
